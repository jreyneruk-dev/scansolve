...
function Intro(){
  return <Sprite start={0} end={3.5}>{({localTime:lt, duration})=>{
    ...
    return (
      <div style={{position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:40, ...fx}}>
        <div style={{transform:`scale(${pop(lt,0.1,0.7)})`}}><QRTile size={240} radius={58} light={build}/></div>
        <div style={{fontFamily:FONT, fontWeight:800, fontSize:96, color:C.ink, letterSpacing:'-0.02em',
          opacity:fadeIn(lt,0.5,0.4), transform:`translateY(${(1-fadeIn(lt,0.5,0.4))*18}px)`}}>ScanSolve</div>
        <div style={{fontFamily:FONT, fontWeight:600, fontSize:38, color:C.inkSoft, opacity:fadeIn(lt,0.85,0.4)}}>
    ...
    return (
      <div style={{position:'absolute', inset:0, ...fx}}>
        <div style={{position:'absolute', inset:0, transform:`scale(${cam})`, transformOrigin:'50% 46%'}}>
          ...
          <div style={{position:'absolute', left:0, right:0, bottom:0, height:300, background:C.floor}}/>
          <div style={{position:'absolute', left:0, right:0, bottom:300, height:2, background:'rgba(80,70,140,0.08)'}}/>
          <Particles t={lt} count={14} blur={3}/>
          {/* flickering broken bulb (upper-left) */}
          <div style={{position:'absolute', left:330, top:360, transform:'translateX(-50%) scale(1.3)'}}>
            <Bulb on={bulbFlick}/>
          </div>
          {/* ScanSolve QR poster on wall (upper-right) */}
          <div style={{position:'absolute', left:556, top:330}}>
            <div style={{background:'#fff', borderRadius:18, padding:11, boxShadow:'0 18px 40px -16px rgba(40,40,90,0.35)', border:`1px solid ${C.line}`}}>
              <img src="/scansolve-anim-poster.png" alt="ScanSolve poster" style={{display:'block', width:330, height:'auto', borderRadius:8}}/>
            </div>
            {/* scan sweep tint over the QR while scanning */}
            {sweep>0 && sweep<1 && (
              <div style={{position:'absolute', left:200, top:18, width:138, height:`${sweep*210}px`, maxHeight:210,
                background:'linear-gradient(180deg, rgba(107,91,230,0.0), rgba(107,91,230,0.22))', borderRadius:5, pointerEvents:'none'}}/>
            )}
            {/* detection rings over the QR on capture */}
            {captured && lt<3.5 && [0,1].map(i=>(
              <div key={i} style={{position:'absolute', left:264, top:128, width:90, height:90, borderRadius:'50%',
                border:`3px solid ${C.green}`, transform:`translate(-50%,-50%) scale(${pop(lt,2.6+i*0.12,0.6)*1.9})`,
                opacity:clamp(1-(lt-2.6-i*0.12)/0.8,0,1)}}/>
            ))}
          </div>
          {/* phone — scan mode, then survey — large, lower-center */}
          {surveyIn<1 && (
            <ScanPhone sweep={sweep} captured={captured} w={376}
              style={{left:540, top:770, opacity:clamp(1-surveyIn*1.4,0,1),
                transform:`translateX(-50%) translate(${shake}px, ${phoneY+bob}px) scale(${tween(surveyIn,0,1,1,1.04)})`,
                transformOrigin:'center'}}/>
          )}
          {surveyIn>0 && (
            <PhoneSurvey reveal={reveal} w={406}
              style={{left:540, top:730, opacity:clamp(surveyIn*1.6,0,1),
                transform:`translateX(-50%) translateY(${bob*0.4}px) scale(${tween(surveyIn,0,1,0.92,1)})`,
                transformOrigin:'center'}}/>
          )}
          {/* finger tap on selected chip */}
          {tapShow && (
            <div style={{position:'absolute', left:440, top:1210}}>
              <div style={{position:'absolute', width:74, height:74, borderRadius:'50%', border:`3px solid ${C.violet}`,
                transform:`translate(-50%,-50%) scale(${pop(lt,6.2,0.5)*1.7})`, opacity:clamp(1-(lt-6.2)/0.7,0,1)}}/>
              <div style={{position:'absolute', width:42, height:42, borderRadius:'50%', background:'rgba(107,91,230,0.85)',
    ...
    return (
      <div style={{position:'absolute', inset:0, ...fx}}>
        <div style={{position:'absolute', inset:0, transform:`scale(${cam})`, transformOrigin:'50% 42%'}}>
          <div style={{position:'absolute', inset:0, background:`linear-gradient(180deg, #F1F0FB, ${C.wall})`}}/>
          <div style={{position:'absolute', left:0, right:0, top:1290, bottom:0, background:'#CFC9E6'}}/>
          <div style={{position:'absolute', left:0, right:0, top:1272, height:20, background:'#B7B0D8'}}/>
          <Particles t={lt} count={12} blur={3}/>
          {/* monitor */}
          <div style={{position:'absolute', left:'50%', top:tween(rise,0,1,720,560)+float, transform:`translateX(-50%) scale(1.46)`, transformOrigin:'top center', opacity:clamp(rise*1.3,0,1)}}>
  ...
  return <Sprite start={19.8} end={27.6}>{({localTime:lt, duration})=>{
    ...
    return (
      <div style={{position:'absolute', inset:0, ...fx}}>
        <div style={{position:'absolute', inset:0, transform:`scale(${cam})`, transformOrigin:'50% 46%'}}>
          <div style={{position:'absolute', inset:0, background:`linear-gradient(180deg, #F1F0FB, ${C.wall})`}}/>
          <div style={{position:'absolute', left:0, right:0, bottom:0, height:360, background:C.floor}}/>
          <div style={{position:'absolute', left:0, right:0, bottom:360, height:2, background:'rgba(80,70,140,0.08)'}}/>
          <Particles t={lt} count={13} blur={3} color="rgba(242,179,60,0.16)"/>
          {/* big warm glow once fixed */}
          <div style={{position:'absolute', left:'50%', top:780, width:940, height:940, borderRadius:'50%',
          ...
          <div style={{position:'absolute', left:'50%', top:780, transform:`translate(-50%,-50%) scale(4.1)`, transformOrigin:'center'}}>
            <Bulb on={bulbOn}/>
          </div>
          {/* resolved card */}
          {cardIn>0 && (
            <div style={{position:'absolute', left:'50%', top:1210, transform:'translateX(-50%) scale(1.3)'}}>
          ...
          {fixed && bulbOn>0.5 && [0,1,2,3,4,5].map(i=>{
            ...
            const rad = 240 + (i%2)*60;
            const ax = Math.cos(ang)*rad, ay = Math.sin(ang)*rad - 40;
            return <div key={i} style={{position:'absolute', left:540+ax, top:740+ay, width:24, height:24,
...
function Outro(){
  return <Sprite start={27.4} end={29}>{({localTime:lt, duration})=>{
    ...
    return (
      <div style={{position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:34, opacity:inT*(1-outT)}}>
        <div style={{transform:`scale(${pop(lt,0,0.6)})`}}><QRTile size={200} radius={48}/></div>
        <div style={{fontFamily:FONT, fontWeight:800, fontSize:84, color:C.ink, letterSpacing:'-0.02em'}}>ScanSolve</div>
        <div style={{display:'flex', gap:22, alignItems:'center', fontFamily:FONT, fontWeight:700, fontSize:36, color:C.violet}}>
...
function Chrome(){
  ...
  return (
    <>
      {/* header (top center) */}
      <div style={{position:'absolute', left:'50%', top:58, transform:'translateX(-50%)', display:'flex', alignItems:'center', gap:16, opacity:0.97}}>
        <QRTile size={56} radius={16}/>
        <div style={{fontFamily:FONT, fontWeight:800, fontSize:34, color:C.ink, letterSpacing:'-0.01em'}}>ScanSolve</div>
      </div>
      {/* stage stepper */}
      <div style={{position:'absolute', left:'50%', top:156, transform:'translateX(-50%)', display:'flex', alignItems:'center', gap:16}}>
        {stages.map((s,i)=>{
          const on=i===active;
          return (
            <div key={i} style={{display:'flex', alignItems:'center', gap:11,
              padding:'12px 24px', borderRadius:30, background:on?C.violet:'rgba(255,255,255,0.72)',
              border:`1px solid ${on?C.violet:C.line}`, transition:'none'}}>
              <span style={{fontFamily:FONT, fontWeight:800, fontSize:20, color:on?'#fff':C.inkFaint}}>{s.n}</span>
              <span style={{fontFamily:FONT, fontWeight:700, fontSize:21, color:on?'#fff':C.inkSoft}}>{s.label}</span>
            </div>
          );
        })}
      </div>
      {/* caption bottom */}
      <div style={{position:'absolute', left:'50%', bottom:120, transform:'translateX(-50%)', textAlign:'center', width:'88%'}}>
        <div style={{display:'inline-block', fontFamily:FONT, fontWeight:700, fontSize:32, color:C.ink, background:'rgba(255,255,255,0.8)',
          padding:'18px 36px', borderRadius:44, backdropFilter:'blur(4px)', boxShadow:'0 12px 28px -10px rgba(40,30,90,0.28)', lineHeight:1.25}}>
          {cur.cap}
        </div>
      </div>
    </>
...
function App(){
  return (
    <EmbedStage width={1080} height={1920} duration={29}
...
export { App as ScanSolvePortrait };
